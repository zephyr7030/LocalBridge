...
- 请求诊断统一 `request_id/connection_id/attempt/error_code/phase/cause/http_status/duration_ms`；重试复用同一 `request_id` 并递增 `attempt`，成功重试仍与最初失败关联。request start/end 使用同一 correlation；工程字段只进入脱敏日志/导出，不进入普通诊断 UI。
