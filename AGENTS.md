...
55. Schema27：LocalBridge 必须拥有 public command Session Manager；public `session_id/output_ref` 不得直接等于或泄露 upstream private handle。`command_control` 固定为 `poll(session_id)` live poll、`write(session_id)`、`kill(session_id)`、`read(output_ref)`；session 必须后台收敛 `running→completed/failed/timed_out/cancelled/lost`，private pruning/丢失不得形成永久 Running。非零 exit 即 `ProcessFailed`/Failed task，即使 stdout/stderr 为空也不得 public success。
56. Schema27：存在 active workspace 时 `workspace_context.workspace` 必须是非空 ordinary absolute Win32 path（例如 `D:\project`，禁止 `\\?\`），`default_cwd` 单独为 workspace-relative。除该只读 projection 外，`exec_command.workdir`、Git/document/image 等 workspace-bound public path 输入统一 active-workspace-relative；drive/UNC/verbatim/POSIX absolute 与 `..` traversal 在 LocalBridge boundary typed reject，normalization 不得扩大授权。
57. Schema27：public schema 只能广告真实可执行 action；当前 `agent_workflow` 九 action、`task_control get/cancel`、`document_workflow inspect/create/convert/rebuild` 必须在 LB-006 PASS 前可用。adapter 消费但 upstream outputSchema 未精确保证的 result 字段必须在 facade 启动前做 deterministic semantic probe。`git_workflow` 五 action 共享 workspace-bounded nested-repository resolver；`git_status` 能识别的 nested repo 必须被 log/show/diff/blame 一致识别，已发现 repo 时 diff 禁止 non-git fallback。
58. Schema28 测试编排固定三层：`PR Fast Gate` 只跑当前 PR 的廉价 deterministic unit/fake/static targeted checks；`PR Runtime Gate` 才运行真实 bundled runtime/PEP/process，且相同外部拓扑必须按类压缩到共享 lifecycle，重复启动必须说明 isolation 必要性，廉价 unit 不得被强行合成巨型测试；`Group/Release Gate` 才运行完整 cargo/npm/build/clippy/architecture/release 链。开发内循环禁止每个小改后重复全仓 Gate。静态 source-marker 只能证明静态合同，不能替代可执行行为，也不能因为实现/测试函数名存在就宣告行为 PASS。所有 runner 必须 bounded timeout/cancel；lost session 必须终态失败，不能无限 running。
59. Schema28 LB-006 Public Runtime：shell command 只允许一次预期 shell parse，`Write-Output "a|b"` / `"a&b"` 与单引号等价语义必须保真；`command_control.poll` 返回 ordered incremental delta、不得丢/重放，`write` 必须可操作 exec 后仍 running 的 public session，`kill` 在 healthy runtime 下不得 `RuntimeUnavailable` 且成功后稳定收敛 cancelled、后续 poll 不得退化 `SessionUnavailable`。`view_image auto_resize` 必须真实缩放；Windows PowerShell/auto(PowerShell) 中文输出必须 UTF-8；Git blame/document 范围为 1-based inclusive，`start_line > end_line` 必须 `InvalidArgument`。adapter 消费但 outputSchema 未保证的 private result 字段全部需要 serving 前确定性语义 probe 或等价 fail-closed 证明。
60. 开发/测试期允许后台命令进程或可见命令窗口，其存在本身不是产品缺陷，也不要求开发环境完全无 console。正式打包/正常 GUI LocalBridge 中，由 LocalBridge 自己拥有并启动的 bundled coding runtime、PEP-adjacent managed command route、Tunnel、Broker/background helper、shell/direct command 子进程不得意外创建可见 console window（除非未来显式 interactive-terminal 合同）；必须通过 release-style/packaged launcher 实测，不能从开发 harness 窗口反推 PASS/FAIL，并继续保持 Job/process ownership 与最小必要进程拓扑。
61. Schema29：command/session 一旦进入 terminal（completed/failed/timed_out/cancelled/lost 或等价 terminal error），必须经过一个不可跳过的 `finally` / finally-equivalent finalizer。在同一个 task-state owner transaction 内，finalizer 必须**原子地**写入且只写入一次 `command_finished` terminal event，并清空 `current_command`；成功、非零退出、timeout、cancel、kill、runtime error、工具异常等所有退出路径都适用。禁止先清空后写事件、只写事件不清空、只清空不落 terminal snapshot，或让 terminal task 长期保留 running `current_command`。
62. Schema29：task-state 自己保存 bounded/redacted terminal command snapshot，必须在 private runtime session 被 prune/过期后仍能确定最终状态；约 300 秒 private session retention 只能用于临时输出读取，不得作为最终结果真相源。command start/replace/finish/clear 必须以 `(task_id, session_id)` 做 compare-and-swap/owner check；旧 task/旧 session 的延迟 finalizer 不得覆盖或清除新 owner，owner mismatch 只能 no-op 或 typed conflict。重复 terminal callback 必须幂等，不重复 `command_finished`，不复活 `current_command`。
63. Schema29：正常前台首次创建并显示主窗口时，固定 780×620 窗口默认居中于当前 monitor work area，允许 DPI 整数取整误差；这是 LB-015 UI Shell 合同。托盘重新显示一个已经存在且可能被用户移动过的隐藏窗口时不得强制重新居中。
64. Schema30：`agent_workflow` 必须接受 active-workspace-relative nested-project `path`；`path=LocalBridge`/其后代必须与 `git_workflow` 使用一致的 enclosing-repo discovery，最多向上到 active workspace root。该 selector 只改变 workflow/project context，永远不得切换 WorkspaceRegistry 或 active authorization root。
65. Schema30：可信 `windows_powershell` 与 `auto`→PowerShell 必须保留最小标准 coding cmdlet surface（至少 `Get-Location` / `Get-ChildItem` / `Test-Path`）。arbitrary module autoload 继续关闭；标准模块只能来自固定/身份验证的可信系统来源，用户控制 `PSModulePath` 不得重定向 preload。不得为修可用性而放宽 LB-007 对 provider/dynamic-command mutation 的 review-required 边界。
66. Schema30：active workspace 是 reviewed read/write 授权根，不是只读根。结构化目录能力固定为 `agent_workflow.directory_changes[]`，每项仅 `{action,path}`，action 只允许 `create_directory` / `remove_empty_directory`，path active-workspace-relative；无需 process exec，Edit/Full 可在授权根内创建测试目录并在为空时清理。absolute/`..`/reparse escape deny，禁止借本合同扩成非空递归 tree delete，WorkspaceRegistry/active workspace/control-plane 永久不可由 MCP 修改。
67. Project coding skill：任何 coding / bugfix / refactor / review / design / dependency-selection 任务，在理解任务并追踪真实代码路径后，必须读取并应用 `skills/ponytail/SKILL.md`，默认强度 `full`；用户可显式切换 `lite/full/ultra` 或用 `stop ponytail` / `normal mode` 停用。Ponytail 只负责减少不必要实现与依赖，永远不得简化掉显式需求、输入/信任边界校验、防数据丢失错误处理、安全措施、可访问性、当前 PR writable paths、必要测试，且不得覆盖 `first-principles` / `maintainable-code` 与本文件其他硬合同；冲突时以后者和更高安全/治理约束为准。
