...
4. **安全/架构硬边界**：Public API 与 policy fail-closed；Edit/Full 的 workspace 权限不得越过 active root；Elevated 仅在固定风险警告 + 9 秒红色确认倒计时 + 用户明确确认 + Windows UAC + Broker Active 后，才可在管理员 Token 范围内越过 active root 执行全文件系统、普通/管理员进程命令与系统维护；LocalBridge control-plane 永久 deny；secret 不得明文泄漏；管理员能力只走既定 Broker/UAC 边界；Rust/backend 持有 lifecycle/readiness/retry/权限/CurrentTask truth，前端只做 typed projection + typed intent。
