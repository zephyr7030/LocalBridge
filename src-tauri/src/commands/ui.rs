...
#[derive(Debug, Clone, PartialEq, Eq, Serialize)]
#[serde(rename_all = "camelCase")]
struct ProjectProjection { id: String, path: String, active: bool }

#[derive(Debug, Clone, PartialEq, Eq, Serialize)]
#[serde(rename_all = "camelCase")]
struct TaskProjection { kind: &'static str, summary: Option<String>, state: &'static str }

#[derive(Debug, Clone, PartialEq, Eq, Serialize)]
struct ReconnectProjection { generation: u64 }

#[tauri::command]
pub fn get_main_projection(app: AppHandle, lifecycle: State<'_, DesktopLifecycle>) -> Result<MainProjection, String> {
    ...
    let active_id = data.workspace.active_workspace_id.as_ref().map(WorkspaceId::as_str);
    let projects = data.workspace.remembered_entries().iter().map(|entry| ProjectProjection {
    ...
    let current_project = data.workspace.active_entry().map(|entry| entry.display_path.to_string_lossy().into_owned());
    let (tunnel_service, coding_service) = service_codes(&snapshot.state);
    let reconnect = snapshot.outage.and_then(|outage| outage.user_attention_required.then_some(ReconnectProjection { generation: outage.generation }));
    Ok(MainProjection {
        permission: stored_permission_code(data.settings.permission_mode),
        privilege: privilege_code(&privilege), tunnel_service, coding_service, current_project, projects,
        current_task: task_projection(&snapshot.current_task), runtime_key_saved: metadata.has_runtime_key,
        auto_start: data.settings.auto_start_services, reconnect,
    })
}

#[tauri::command]
pub fn set_permission_mode(mode: String, app: AppHandle, lifecycle: State<'_, DesktopLifecycle>) -> Result<(), String> {
    ...
    if requested == previous { return Ok(()); }
    if previous == PermissionMode::Elevated && requested != PermissionMode::Elevated {
        lifecycle.privilege().disable().map_err(|_| "无法关闭管理员权限".to_string())?;
    }
    let runtime_active = lifecycle.runtime_snapshot().active;
    if runtime_active {
        lifecycle.set_runtime_permission_mode(requested).map_err(|_| "无法更新当前权限模式".to_string())?;
    }
    data.settings.permission_mode = requested.into();
    if store.save(&data).is_err() {
        if runtime_active { let _ = lifecycle.set_runtime_permission_mode(previous); }
...
pub fn set_auto_start(enabled: bool, app: AppHandle) -> Result<(), String> {
    ...
    let manager = AutostartManager::for_current_executable().map_err(|_| "无法读取开机启动设置".to_string())?;
    manager.set_enabled(enabled).map_err(|_| "无法更新开机启动设置".to_string())?;
...
pub fn delete_runtime_key() -> Result<(), String> {
    WindowsCredentialStore::default().delete_runtime_api_key().map_err(|_| "无法删除运行密钥".to_string())?;
    Ok(())
}
...
pub fn enable_admin(app: AppHandle, lifecycle: State<'_, DesktopLifecycle>) -> Result<(), String> {
    let (_, data) = load_app_data(&app)?;
    if data.settings.permission_mode != StoredPermissionMode::Elevated { return Err("请先选择管理员模式".to_string()); }
    let executable = std::env::current_exe().map_err(|_| "无法定位管理员组件".to_string())?;
    let broker = executable.parent().map(|parent| parent.join("localbridge-privileged-broker.exe")).ok_or_else(|| "无法定位管理员组件".to_string())?;
    lifecycle.privilege().enable_from_explicit_user_action(&broker).map_err(|_| "管理员授权未完成".to_string())?;
    Ok(())
}

#[tauri::command]
pub fn disable_admin(lifecycle: State<'_, DesktopLifecycle>) -> Result<(), String> {
    lifecycle.privilege().request_without_uac().map_err(|_| "无法关闭管理员权限".to_string())
}

#[tauri::command]
pub fn retry_connection(lifecycle: State<'_, DesktopLifecycle>) -> Result<(), String> {
    lifecycle.manual_retry_after_attention().map_err(|_| "当前连接无法重试".to_string())?; Ok(())
}
...
#[tauri::command]
pub fn select_project(id: String, app: AppHandle, lifecycle: State<'_, DesktopLifecycle>) -> Result<(), String> {
    let id = WorkspaceId::from_validated(id).map_err(|_| "项目不存在".to_string())?;
    let (store, mut data) = load_app_data(&app)?;
    let entry = data.workspace.registry.get(&id).cloned().ok_or_else(|| "项目不存在".to_string())?;
    let validated = WorkspaceValidator.validate(&entry.display_path).map_err(|_| "项目已无法访问".to_string())?;
    if entry.validated_identity.as_str() != validated.identity().as_str() { return Err("项目身份已变化，请重新添加".to_string()); }
    activate_project(&app, &lifecycle, &store, &mut data, id, validated.resolved_path())
}

#[tauri::command]
pub fn remove_project(id: String, app: AppHandle, lifecycle: State<'_, DesktopLifecycle>) -> Result<(), String> {
    let id = WorkspaceId::from_validated(id).map_err(|_| "项目不存在".to_string())?;
    let (store, mut data) = load_app_data(&app)?;
    let original = data.clone();
    if data.workspace.registry.get(&id).is_none() { return Err("项目不存在".to_string()); }
    ...
    if was_active && runtime_before.active {
        lifecycle.stop_runtime_for_control_plane().map_err(|_| "无法停止当前项目服务".to_string())?;
    }
    if was_active { data.workspace.clear_active(); }
    let _ = data.workspace.registry.remove(&id);
    if store.save(&data).is_err() {
        if was_active && runtime_before.active {
            if let Some(path) = runtime_before.configured_workspace.as_deref() { let _ = start_runtime_for_path(&app, &lifecycle, &original, path); }
        }
...
fn activate_project(app: &AppHandle, lifecycle: &DesktopLifecycle, store: &SettingsStore, data: &mut AppData, id: WorkspaceId, candidate: &Path) -> Result<(), String> {
    clear_manual_stop_for_explicit_action(app)?;
    let before = lifecycle.runtime_snapshot();
    let already_current = before.configured_workspace.as_deref().is_some_and(|path| path == candidate);
    if !already_current {
        if before.active {
            lifecycle.switch_runtime_workspace(candidate, before.configured_workspace.as_deref()).map_err(|_| "无法切换到所选项目".to_string())?;
        } else {
            start_runtime_for_path(app, lifecycle, data, candidate)?;
        }
    }
    data.workspace.set_active_reference(id).map_err(|_| "无法设置当前项目".to_string())?;
    if store.save(data).is_err() {
        if !already_current {
            if let Some(previous) = before.configured_workspace.as_deref() { let _ = lifecycle.switch_runtime_workspace(previous, Some(candidate)); }
            else if lifecycle.runtime_snapshot().active { let _ = lifecycle.stop_runtime_for_control_plane(); }
        }
...
fn start_runtime_for_path(app: &AppHandle, lifecycle: &DesktopLifecycle, data: &AppData, path: &Path) -> Result<(), String> {
...
fn clear_manual_stop_for_explicit_action(app: &AppHandle) -> Result<(), String> {
    ...
    if profile.manual_stop_latched() {
        profile.clear_manual_stop(); store.save(&profile).map_err(|_| "无法保存连接设置".to_string())?;
    }
    Ok(())
}
...
fn load_app_data(app: &AppHandle) -> Result<(SettingsStore, AppData), String> {
    ...
    let data = store.load().map_err(|_| "无法读取设置".to_string())?; Ok((store, data))
}
...
fn new_workspace_id() -> String { format!("ui-{}-{}", std::process::id(), unix_nanos()) }
fn unix_seconds() -> u64 { SystemTime::now().duration_since(UNIX_EPOCH).unwrap_or_default().as_secs() }
fn unix_nanos() -> u128 { SystemTime::now().duration_since(UNIX_EPOCH).unwrap_or_default().as_nanos() }

fn parse_permission(value: &str) -> Result<PermissionMode, String> {
    match value { "edit" => Ok(PermissionMode::Edit), "full" => Ok(PermissionMode::Full), "admin" => Ok(PermissionMode::Elevated), _ => Err("权限模式无效".to_string()) }
}
fn stored_permission_code(value: StoredPermissionMode) -> &'static str {
    match value { StoredPermissionMode::Edit => "edit", StoredPermissionMode::Full => "full", StoredPermissionMode::Elevated => "admin" }
}
fn privilege_code(value: &PrivilegeState) -> &'static str {
    match value { PrivilegeState::Disabled => "off", PrivilegeState::Requested => "requested", PrivilegeState::AwaitingUac => "awaiting", PrivilegeState::Active { .. } => "active", PrivilegeState::Faulted(_) => "fault" }
}
fn service_codes(state: &RuntimeState) -> (&'static str, &'static str) {
    match state {
        RuntimeState::Stopped => ("off", "off"),
        RuntimeState::StartingMcp | RuntimeState::WaitingMcpReady | RuntimeState::StartingPolicyEnforcement | RuntimeState::WaitingPolicyReady => ("off", "starting"),
        RuntimeState::StartingTunnel | RuntimeState::WaitingTunnelReady => ("starting", "online"),
        RuntimeState::Ready => ("online", "online"),
        RuntimeState::Recovering { component, .. } => match component { RuntimeComponent::Tunnel => ("recovering", "online"), RuntimeComponent::PolicyEnforcement | RuntimeComponent::CodingRuntime => ("recovering", "recovering") },
...
fn task_projection(status: &CurrentTaskStatus) -> Option<TaskProjection> {
    let CurrentTaskStatus::Active(task) = status else { return None; };
    Some(TaskProjection { kind: task_kind_code(task.kind), summary: task.summary.as_deref().map(str::to_owned), state: task_state_code(task.state) })
}
fn task_kind_code(kind: TaskKind) -> &'static str {
    match kind { TaskKind::ReadFile => "read", TaskKind::SearchCode => "search", TaskKind::ModifyFile => "modify", TaskKind::ExecuteCommand => "command", TaskKind::GitOperation => "git", TaskKind::Build => "build", TaskKind::Test => "test", TaskKind::ElevatedOperation => "admin", TaskKind::Other => "other" }
}
fn task_state_code(state: TaskExecutionState) -> &'static str {
    match state { TaskExecutionState::Idle => "idle", TaskExecutionState::Running => "running", TaskExecutionState::AwaitingAuthorization => "waiting", TaskExecutionState::Blocked => "blocked", TaskExecutionState::Failed => "failed", TaskExecutionState::Cancelled => "cancelled" }
}
