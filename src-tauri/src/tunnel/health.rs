...
pub(crate) struct HealthEndpoint { port: u16 }
...
impl HealthEndpoint {
    ...
    pub(crate) fn probe_ready(&self) -> Result<bool, TunnelError> {
        if get(self.port, "/readyz")?.status != 200 { return Ok(false); }
...
fn get(port: u16, path: &str) -> Result<HttpResponse, TunnelError> {
    ...
    stream.take((MAX_RESPONSE + 1) as u64).read_to_end(&mut bytes).map_err(|_| TunnelError::HealthUnavailable)?;
    if bytes.len() > MAX_RESPONSE { return Err(TunnelError::HealthProtocol); }
...
fn parse_response(bytes: &[u8]) -> Result<HttpResponse, TunnelError> {
    ...
    let status = lines.next().and_then(|line| line.split_whitespace().nth(1)).and_then(|value| value.parse::<u16>().ok()).ok_or(TunnelError::HealthProtocol)?;
    let mut chunked = false;
    let mut content_length = None;
    for line in lines {
        if let Some((name, value)) = line.split_once(':') {
            if name.eq_ignore_ascii_case("Transfer-Encoding") && value.trim().eq_ignore_ascii_case("chunked") { chunked = true; }
    ...
    if let Some(expected) = content_length { if expected != body.len() { return Err(TunnelError::HealthProtocol); } }
...
fn decode_chunked(mut input: &[u8]) -> Result<Vec<u8>, TunnelError> {
    ...
    loop {
        ...
        let size_text = std::str::from_utf8(&input[..line_end]).map_err(|_| TunnelError::HealthProtocol)?;
        let size = usize::from_str_radix(size_text.split(';').next().unwrap_or(""), 16).map_err(|_| TunnelError::HealthProtocol)?;
...
mod tests {
    ...
    fn fake_health(status_body: &'static str) -> (HealthEndpoint, JoinHandle<()>) {
        ...
        let handle = thread::spawn(move || {
            for _ in 0..2 {
                ...
                let body = if request.starts_with("GET /api/status ") { status_body } else { "" };
                let response = format!(
                    "HTTP/1.1 200 OK\r\nContent-Length: {}\r\nConnection: close\r\n\r\n{}",
                    body.len(), body
                );
    ...
    fn ready_requires_control_plane_metadata_not_readyz_alone() {
        ...
        assert!(matches!(
            health.probe_ready(),
            Err(TunnelError::ControlPlane(super::super::fault::ControlPlaneFault::Network))
        ));
        server.join().unwrap();
    }
    ...
    fn authentication_error_is_typed_non_recoverable() {
        ...
        assert_eq!(error.retryability(), super::super::fault::Retryability::NonRecoverable);
        server.join().unwrap();
    }
