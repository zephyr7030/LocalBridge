...
const artifacts = resolve(root, "release-artifacts/LB-018");
...
function generateSbom() {
  ...
  json("sbom.cdx.json", { bomFormat: "CycloneDX", specVersion: "1.5", version: 1, metadata: { component: { type: "application", name: "LocalBridge", version: "0.1.0" } }, components });
}
...
function emitEvidence() {
  ...
  json("release-provenance.json", {
    schema: 1, product: "LocalBridge", version: "0.1.0", target: "windows-11-x86_64", source_commit: inventory.source_commit,
    ...
    no_console_evidence: { localbridge_pe_subsystem: mainSubsystem, broker_pe_subsystem: brokerSubsystem, managed_runtime_supervisor_uses_CREATE_NO_WINDOW: readFileSync(resolve(root, "src-tauri/src/runtime/windows_supervisor.rs"), "utf8").includes("CREATE_NO_WINDOW"), privileged_execution_uses_CREATE_NO_WINDOW: readFileSync(resolve(root, "src-tauri/src/privilege/execution.rs"), "utf8").includes("CREATE_NO_WINDOW"), coding_runtime_processes_use_CREATE_NO_WINDOW: readFileSync(resolve(root, "runtime/coding-tools-mcp/coding_tools_mcp/processes.py"), "utf8").includes("CREATE_NO_WINDOW") },
...
function build() {
  ...
  const result = emitEvidence();
  console.log(`LB018_RELEASE_BUILD=PASS installer=${result.installer} size_mib=${result.size.installer_mib} installed_payload_mib=${result.size.installed_payload_mib}`);
}

function refresh() {
  const result = emitEvidence();
  console.log(`LB018_RELEASE_REFRESH=PASS installer=${result.installer} source_commit=${run("git", ["rev-parse", "HEAD"]).trim()}`);
}
...
function verify() {
  ...
  console.log("LB018_RELEASE_VERIFY=PASS cloudflared=false webview2=system sbom=true provenance=true no_console=true");
}
