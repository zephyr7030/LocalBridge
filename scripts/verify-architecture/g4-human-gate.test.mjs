...
const schema43Contracts = JSON.parse(readFileSync(new URL("../../PR_CONTRACTS.json", import.meta.url), "utf8"));
assert.equal(hasExactSchema43V011NoConsoleFilesystemAmendment20260819(schema43Contracts), true);
const schema43MissingHash=structuredClone(schema43Contracts); schema43MissingHash.rules.filesystem_actions=schema43MissingHash.rules.filesystem_actions.filter(x=>x!=="hash"); assert.equal(hasExactSchema43V011NoConsoleFilesystemAmendment20260819(schema43MissingHash),false);
const schema42Contracts = normalizeSchema43V011NoConsoleFilesystemAmendment20260819(schema43Contracts);
assert.equal(hasExactLb018CloudflareTestScopeCorrection20260819(schema42Contracts), true);
const schema42MissingLb018TunnelTestScope = structuredClone(schema42Contracts);
schema42MissingLb018TunnelTestScope.prs["LB-018"].writable_paths = schema42MissingLb018TunnelTestScope.prs["LB-018"].writable_paths.filter((item) => item !== "tests/integration/tunnel/lb008_contract.test.mjs");
assert.equal(hasExactLb018CloudflareTestScopeCorrection20260819(schema42MissingLb018TunnelTestScope), false);
const schema42WithoutLb018TunnelTestScope = normalizeLb018CloudflareTestScopeCorrection20260819(schema42Contracts);
assert.equal(schema42WithoutLb018TunnelTestScope.prs["LB-018"].writable_paths.includes("tests/integration/tunnel/lb008_contract.test.mjs"), false);
...
const schema42BoundedDiagnosticsDrift=structuredClone(schema42Contracts); schema42BoundedDiagnosticsDrift.rules.request_diagnostics_active_tracking_limit=0; assert.equal(hasExactSchema42UnifiedErrorDiagnosticsAmendment20260819(schema42BoundedDiagnosticsDrift),false);
const schema42RecoveryPathDrift=structuredClone(schema42Contracts); schema42RecoveryPathDrift.prs["LB-017"].writable_paths=schema42RecoveryPathDrift.prs["LB-017"].writable_paths.filter(x=>x!=="src-tauri/src/runtime/recovery.rs"); assert.equal(hasExactSchema42UnifiedErrorDiagnosticsAmendment20260819(schema42RecoveryPathDrift),false);
const schema42RecoveryExportPathDrift=structuredClone(schema42Contracts); schema42RecoveryExportPathDrift.prs["LB-017"].writable_paths=schema42RecoveryExportPathDrift.prs["LB-017"].writable_paths.filter(x=>x!=="src-tauri/src/runtime/mod.rs"); assert.equal(hasExactSchema42UnifiedErrorDiagnosticsAmendment20260819(schema42RecoveryExportPathDrift),false);
...
const schema42WindowsContracts = normalizeSchema42ToolboxAmendment20260818(schema42ToolboxContracts);
assert.equal(hasExactSchema42WindowsExecutionPolicyAmendment20260818(schema42WindowsContracts), true);
const schema42WindowsDrift=structuredClone(schema42WindowsContracts); schema42WindowsDrift.rules.full_descendant_process_system_management_blocking_not_claimed_without_os_confinement=false; assert.equal(hasExactSchema42WindowsExecutionPolicyAmendment20260818(schema42WindowsDrift),false);
const schema42DashboardContracts = normalizeSchema42WindowsExecutionPolicyAmendment20260818(schema42WindowsContracts);
assert.equal(hasExactSchema42DashboardObservabilityAmendment20260818(schema42DashboardContracts), true);
const schema42DashboardDrift=structuredClone(schema42DashboardContracts); schema42DashboardDrift.rules.dashboard_last_activity_prefix="上次执行工具："; assert.equal(hasExactSchema42DashboardObservabilityAmendment20260818(schema42DashboardDrift),false);
const schema42DashboardStaleIdle=structuredClone(schema42DashboardContracts); schema42DashboardStaleIdle.prs["LB-015"].required_tests=schema42DashboardStaleIdle.prs["LB-015"].required_tests.map(x=>x.startsWith("Dashboard no-task first row is always visible as 空闲")?"Dashboard no-task first row is always visible as 等待命令 and never 空闲; relative age is not appended to this first row":x); assert.equal(hasExactSchema42DashboardObservabilityAmendment20260818(schema42DashboardStaleIdle),false);
const schema42DashboardStalePrefix=structuredClone(schema42DashboardContracts); schema42DashboardStalePrefix.prs["LB-015"].required_tests=schema42DashboardStalePrefix.prs["LB-015"].required_tests.map(x=>x.startsWith("a second row displays 上次执行：")?"a second row displays 上次执行工具： followed by a secret-redacted user-facing tool label or safe summary and places the backend-grounded relative age at the far right":x); assert.equal(hasExactSchema42DashboardObservabilityAmendment20260818(schema42DashboardStalePrefix),false);
const schema42BaseContracts = normalizeSchema42DashboardObservabilityAmendment20260818(schema42DashboardContracts);
assert.equal(hasExactSchema42TaskCommandTruth20260818(schema42BaseContracts), true);
const schema42Drift=structuredClone(schema42BaseContracts); schema42Drift.rules.command_summary_derived_from_structured_status=false; assert.equal(hasExactSchema42TaskCommandTruth20260818(schema42Drift),false);
const schema41Contracts = normalizeSchema42TaskCommandTruth20260818(schema42BaseContracts);
assert.equal(schema41Contracts.schema_version,41);

assert.equal(hasExactSchema41CodingAgentCompatibility20260817(schema41Contracts), true);
const schema41RuleDrift = structuredClone(schema41Contracts);
schema41RuleDrift.rules.coding_agent_embedded_llm_forbidden = false;
assert.equal(hasExactSchema41CodingAgentCompatibility20260817(schema41RuleDrift), false);
const schema41ArtifactDrift = structuredClone(schema41Contracts);
schema41ArtifactDrift.prs["LB-006"].required_artifacts = schema41ArtifactDrift.prs["LB-006"].required_artifacts.filter((item) => !item.startsWith("machine-verifiable coding-agent-v1"));
assert.equal(hasExactSchema41CodingAgentCompatibility20260817(schema41ArtifactDrift), false);
const schema40Contracts = normalizeSchema41CodingAgentCompatibility20260817(schema41Contracts);
assert.equal(schema40Contracts.schema_version, 40);
assert.equal(schema40Contracts.rules.localbridge_agent_api_revision, 40);
assert.equal(hasExactSchema40LiveRuntimeHealth20260817(schema40Contracts), true);
const schema40RuleDrift = structuredClone(schema40Contracts);
schema40RuleDrift.rules.runtime_ready_process_liveness_alone_forbidden = false;
assert.equal(hasExactSchema40LiveRuntimeHealth20260817(schema40RuleDrift), false);
const schema40RecoveryTestDrift = structuredClone(schema40Contracts);
schema40RecoveryTestDrift.prs["LB-010"].required_tests = schema40RecoveryTestDrift.prs["LB-010"].required_tests.filter((item) => !item.startsWith("while the supervised coding-tools root process remains alive"));
assert.equal(hasExactSchema40LiveRuntimeHealth20260817(schema40RecoveryTestDrift), false);
const schema39Contracts = normalizeSchema40LiveRuntimeHealth20260817(schema40Contracts);
assert.equal(schema39Contracts.schema_version, 39);
assert.equal(schema39Contracts.rules.localbridge_agent_api_revision, 39);
assert.equal(hasExactSchema39ExecutionPlatform20260817(schema39Contracts), true);
const schema39RuleDrift = structuredClone(schema39Contracts);
schema39RuleDrift.rules.workflow_resume_durable_recovery_required = false;
assert.equal(hasExactSchema39ExecutionPlatform20260817(schema39RuleDrift), false);
const schema39ArtifactDrift = structuredClone(schema39Contracts);
schema39ArtifactDrift.prs["LB-006"].required_artifacts = schema39ArtifactDrift.prs["LB-006"].required_artifacts.filter((item) => !item.startsWith("durable workflow resume checkpoint"));
assert.equal(hasExactSchema39ExecutionPlatform20260817(schema39ArtifactDrift), false);
const schema39TestDrift = structuredClone(schema39Contracts);
schema39TestDrift.prs["LB-006"].required_tests = schema39TestDrift.prs["LB-006"].required_tests.filter((item) => !item.startsWith("the actual downstream MCP client projection"));
assert.equal(hasExactSchema39ExecutionPlatform20260817(schema39TestDrift), false);
const schema38Contracts = normalizeSchema39ExecutionPlatform20260817(schema39Contracts);
assert.equal(schema38Contracts.schema_version, 38);
assert.equal(schema38Contracts.rules.localbridge_agent_api_revision, 38);
assert.equal(hasExactSchema38PublicControlRepair20260817(schema38Contracts), true);
const schema38Drift = structuredClone(schema38Contracts);
schema38Drift.rules.task_control_cancel_detached_public_session_required = false;
assert.equal(hasExactSchema38PublicControlRepair20260817(schema38Drift), false);
const schema37Contracts = normalizeSchema38PublicControlRepair20260817(schema38Contracts);
assert.equal(hasExactSchema37ContractCleanup20260817(schema37Contracts), true);
const schema37Drift = structuredClone(schema37Contracts);
schema37Drift.rules.workspace_safe_ordinary_win32_absolute_input_allowed = false;
assert.equal(hasExactSchema37ContractCleanup20260817(schema37Drift), false);
const schema36Contracts = normalizeSchema37ContractCleanup20260817(schema37Contracts);
