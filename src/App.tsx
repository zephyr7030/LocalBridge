...
import { accessText, currentActivityDetail, currentActivityElapsed, currentActivityText, formatLastToolAge, lastActivityAction, lastActivityOutcome, permissionReconciliationText, projectionStatusText, serviceText, uiText, updateStatusText } from "./presentation";
...
function Dashboard({ onOpenWelcome }: { onOpenWelcome: () => void }) {
  ...
  return <main className="shell">
    ...
    <section className="card">
      <div className="row"><span className="label">当前项目</span><div className="project-actions"><span className={`value ${adminModeFullAccess ? "full-access" : ""}`}>{adminModeFullAccess ? "全目录访问" : projection?.settingsStatus !== "ready" ? projection ? projectionStatusText(projection.settingsStatus) : "正在读取" : activeProject?.path ?? "未选择项目"}</span><button className="secondary" disabled={projection?.settingsStatus !== "ready"} onClick={openProjectPicker}>{activeProject ? "切换" : "选择项目"}</button></div></div>
    ...
    {view === "settings" && <div className="sheet-backdrop" onMouseDown={() => setView("main")}><section className="sheet" onMouseDown={(event) => event.stopPropagation()}><h2>{uiText.settings}</h2>
      ...
      <section className="settings-section"><h3>连接</h3>
        <div className="field"><label htmlFor="tunnel-id">Tunnel ID</label>{editingTunnel ? <><input id="tunnel-id" autoComplete="off" value={tunnelValue} onChange={(event) => setTunnelValue(event.target.value)} placeholder="输入 Tunnel ID"/><div className="inline-actions"><button className="primary" disabled={!tunnelValue.trim()} onClick={() => void run(async () => { await bridge.saveTunnelId(tunnelValue.trim()); setTunnelValue(""); setEditingTunnel(false); })}>保存</button><button className="secondary" onClick={() => { setTunnelValue(""); setEditingTunnel(false); }}>取消</button></div></> : <div className="settings-summary"><span>{projection?.connectionStatus !== "ready" ? projection ? projectionStatusText(projection.connectionStatus) : "正在读取" : projection.tunnelId ?? "未保存"}</span><button className="secondary settings-replace" disabled={projection?.connectionStatus !== "ready"} onClick={() => { setTunnelValue(projection?.tunnelId ?? ""); setEditingTunnel(true); }}>更换</button></div>}</div>
      ...
      <section className="settings-section"><h3>权限</h3><div className="access-grid">{(["edit", "full", "admin"] as AccessCode[]).map((mode) => <button key={mode} disabled={projection?.authorityStatus !== "ready"} className={`choice ${mode === "admin" ? "admin-choice" : ""} ${projection?.permission === mode ? "selected" : ""}`} onClick={() => chooseAccess(mode)}>{accessText[mode]}</button>)}</div><p className="settings-status">{permissionReconciliationText(projection)}</p></section>
