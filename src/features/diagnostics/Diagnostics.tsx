        ...
        {!snapshot ? <p className="diagnostics-muted">正在检查…</p> : (
          <>
            <div className="diagnostics-section">
              <h3>运行状态</h3>
              <div className="diagnostics-checks">{snapshot.checks.filter((check) => check.code !== "runtime_key").map((check) => (
                <div className="diagnostics-check" key={check.code}>
                  <span className={`diagnostics-dot ${check.level}`} aria-hidden="true" />
                  <div><strong>{check.label}</strong><small>{check.detail}</small></div>
                </div>
              ))}</div>
              <div className="diagnostics-check"><span className={`diagnostics-dot ${snapshot.broker.state === "active" ? "ok" : snapshot.broker.state === "fault" ? "error" : "warning"}`} aria-hidden="true" /><div><strong>管理员权限</strong><small>{brokerText[snapshot.broker.state]}</small></div></div>
            </div>
            <div className="diagnostics-section">
              <h3>项目</h3>
