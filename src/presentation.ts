import type { AccessCode, LastToolProjection, PrivilegeCode, ServiceCode, TaskProjection } from "./bridge";
...
export function taskText(task: TaskProjection | null): string { if (!task || task.state === "idle") return "等待命令"; const parts: string[] = [taskKindText[task.kind]]; if (task.summary) parts.push(task.summary); if (task.state === "running" && task.elapsedMs != null) parts.push(formatElapsed(task.elapsedMs)); const state = taskStateText[task.state]; if (state) parts.push(state); return parts.join("  "); }
