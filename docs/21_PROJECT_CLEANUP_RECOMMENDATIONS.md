...
- 公开 main 首次 CI `33880342449` 在 Rust 阶段失败，402/403 项通过，唯一失败是实际 Tunnel 认证头测试；没有在云端直接重跑。
- 产品通过标准 HTTP `Authorization` 头传递认证，但测试把 HTTP 字段名错误地当作大小写敏感，逐字要求 `Authorization`。HTTP 字段名按协议不区分大小写；不同环境返回等价大小写时会被测试误判。
- 测试现按字段名大小写不敏感、字段值精确匹配；仍会在任何请求缺少认证或 Bearer 内容不一致时失败，未放宽安全边界。失败信息不再展开合成凭证。
- 修复后必须先在本地重复运行实际 Tunnel 二进制测试，并重新完成统一门禁、来源证明和安装包构建；只有本地通过才允许第二次快进推送。
