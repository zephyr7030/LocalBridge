...
无活动任务第一行的唯一待机文案为 `等待命令`；禁止 `空闲`，也禁止把相对时间追加到这一行。第二行固定前缀为 `上次执行工具：`，后接脱敏后的用户可理解工具标签/安全摘要，最右侧显示相对时间；禁止 raw MCP tool id。
...
只允许保留一条上一工具元数据，不得演变为 Recent Activity/Timeline/History。
