...
const recovery=readFileSync("src-tauri/src/runtime/recovery.rs","utf8");
...
for(const required of ["pub async fn open_logs","tauri::async_runtime::spawn_blocking","app_data_dir()","materialize_log_directory","Command::new(\"explorer.exe\")"]) if(!commands.includes(required)) throw new Error(`LB-017 materialized log-directory action missing: ${required}`);
if(commands.includes('.join("logs")')) throw new Error("LB-017 open_logs still opens a synthetic empty logs directory");
...
for(const required of ["REQUEST_DIAGNOSTIC_LIMIT", "ACTIVE_REQUEST_DIAGNOSTIC_LIMIT: usize = 32", "RequestDiagnosticEvent", "request_id", "connection_id", "duration_ms", "record_recovery_attempt_event", "RECENT_USER_OBSERVATIONS", "record_recent_transition", "recovery_active", "active_requests", "request_tracking_evicted", "materialize_log_directory", '#[serde(rename = "timestamp")]']) if(!model.includes(required)) throw new Error(`LB-017 schema42 request diagnostics missing: ${required}`);
if(model.includes("record_runtime_request_diagnostics")) throw new Error("LB-017 still derives request diagnostics from sampled runtime state");
for(const required of ["monitor_recovery_with_observer", "record_recovery_attempt_event(&event)", "record_desktop_runtime_events(&snapshot, &monitor_privilege)"]) if(!background.includes(required)) throw new Error(`LB-017 event-driven recovery diagnostics missing: ${required}`);
for(const required of ["monitor_once_with_observer", "RecoveryAttemptEvent::Started", "RecoveryAttemptEvent::Finished", "recover_minimal_cancellable"]) if(!recovery.includes(required)) throw new Error(`LB-017 real recovery attempt boundary missing: ${required}`);
for(const required of ["record_mcp_request_start", "record_mcp_request_result", "record_mcp_request_error", "request_diagnostic_key", "finalize_special_handler_request", "special_handler_aborted"]) if(!server.includes(required)) throw new Error(`LB-017 event-driven MCP request diagnostics missing: ${required}`);
if(server.includes('arguments.get("action").and_then(Value::as_str).ok_or(())?')) throw new Error("LB-017 task_control missing action still has an orphan-producing early return");
...
if(!commands.includes("export_snapshot") || !model.includes("export_snapshot") || !model.includes("materialize_log_directory")) throw new Error("LB-017 redacted export/log materialization path missing");
...
console.log("LB017_CONTRACT=PASS sections=3 actions=3 engineering_details_hidden=true project_path=true transition_dedup=true recovery_attempt_boundary=true bounded_active_requests=32 materialized_log_directory=true export_redacted=true");
