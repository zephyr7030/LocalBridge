...
for(const p of ["src-tauri/src/mcp/policy.rs","src-tauri/src/mcp/guard.rs","tests/integration/policy/policy_enforcement.rs"])if(!existsSync(join(root,p)))process.exit(3);
const policy=read("src-tauri/src/mcp/policy.rs"), guard=read("src-tauri/src/mcp/guard.rs");
...
const decisionMatch=/self\s*\.\s*policy\s*\.\s*decide\s*\(\s*mode\s*,/.exec(guard);
...
console.log("ARCH-005_VERIFY=PASS mandatory_tools_call=true unknown_deny=true control_plane_deny=true mode_rechecked=true");
