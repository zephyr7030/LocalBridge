...
const projectState = JSON.parse(readFileSync("PROJECT_STATE.json", "utf8"));
const startHere = readFileSync("START_HERE.md", "utf8");
const productUx = readFileSync("docs/01_PRODUCT_UX.md", "utf8");
const acceptance = readFileSync("docs/07_ACCEPTANCE.md", "utf8");
const finalReviewDoc = readFileSync("docs/08_FINAL_REVIEW.md", "utf8");
const finalReview = JSON.parse(readFileSync("FINAL_REVIEW.json", "utf8"));
...
const lb015Tests = contracts.prs?.["LB-015"]?.required_tests ?? [];
const currentIdleContract = "Dashboard no-task first row is always visible as 空闲 and never 等待命令; relative age is not appended to this first row";
const staleIdleContract = "Dashboard no-task first row is always visible as 等待命令 and never 空闲; relative age is not appended to this first row";
const currentLastContract = "a second row displays 上次执行： followed by a secret-redacted user-facing tool label or safe summary and places the backend-grounded relative age at the far right";
const staleLastContract = "a second row displays 上次执行工具： followed by a secret-redacted user-facing tool label or safe summary and places the backend-grounded relative age at the far right";
for (const current of [currentIdleContract, currentLastContract]) if (!lb015Tests.includes(current)) throw new Error(`LB-015 current schema42 contract missing: ${current}`);
for (const stale of [staleIdleContract, staleLastContract]) if (lb015Tests.includes(stale)) throw new Error(`LB-015 stale pre-schema42 contract remains: ${stale}`);
if (projectState.current_task_ui?.idle !== "空闲" || projectState.current_task_ui?.last_tool_label !== "上次执行：") throw new Error("LB-015 PROJECT_STATE current task wording is stale");
if (!startHere.includes('首行当前状态/空闲，第二行“上次执行：…”')) throw new Error("LB-015 START_HERE current task wording is stale");
if (!productUx.includes("○  空闲") || !productUx.includes("`上次执行：` 为固定前缀")) throw new Error("LB-015 product UX current task wording is stale");
if (!acceptance.includes('A119 | 主控界面无活动任务 | 第一行固定显示“空闲”') || !acceptance.includes('第二行固定前缀“上次执行：”')) throw new Error("LB-015 acceptance current task wording is stale");
if (!finalReviewDoc.includes("current execution/`空闲`") || !finalReviewDoc.includes("`上次执行：...`")) throw new Error("LB-015 final-review narrative current task wording is stale");
if (finalReview.current_task_status_ui?.idle_text !== "空闲") throw new Error("LB-015 FINAL_REVIEW current idle wording is stale");
for (const required of ["lifecycle.task_aggregate_snapshot()", "current_activity_projection(&task_aggregate)", "last_activity_projection(&task_aggregate)"]) if (!backend.includes(required)) throw new Error(`LB-015 schema42 backend TaskAggregate activity projection missing: ${required}`);
if (!mcp.includes("task_aggregate_snapshot")) throw new Error("LB-015 schema42 PEP TaskAggregate source missing");
for (const required of ["CurrentActivityProjection","LastActivityProjection","progress_current","progress_total","completed_at_ms"]) if (!backend.includes(required)) throw new Error(`LB-015 schema42 typed activity projection missing: ${required}`);
...
if (!bridge.includes("currentActivity: CurrentActivityProjection | null") || !bridge.includes("lastActivity: LastActivityProjection | null")) throw new Error("LB-015 schema42 activity bridge projection missing");
if (!app.includes("const currentActivity = projection?.currentActivity ?? null") || !app.includes("const lastActivity = projection?.lastActivity ?? null") || !app.includes("currentActivityText(currentActivity)") || !app.includes("lastActivityAction(lastActivity)") || !app.includes("lastActivityOutcome(lastActivity)")) throw new Error("LB-015 schema42 frontend is not consuming backend currentActivity/lastActivity");
for (const legacy of ["projection?.currentWorkflow","projection?.currentCommand","projection?.lastCommand","projection?.lastTool"]) if (app.includes(legacy)) throw new Error(`LB-015 frontend still synthesizes schema42 activity from legacy field: ${legacy}`);
...
for (const wording of ['return "空闲"', 'return "任务等待继续"', 'command: "运行命令"', 'git: "Git 操作"']) if (!presentation.includes(wording)) throw new Error(`LB-015 schema42 current activity wording missing: ${wording}`);
if (presentation.includes("结果：") || !app.includes("上次执行：{lastActivityAction(lastActivity)}") || !css.includes(".activity-elapsed,.last-tool-age{grid-column:3")) throw new Error("LB-015 schema42 last-activity natural sentence/age alignment missing");
for(const marker of [".task-row,.last-tool-row{display:grid","font-size:var(--font-body)",".activity-row-main,.last-activity-left{grid-column:2",".activity-summary{min-width:0;max-width:240px;overflow:hidden;text-overflow:ellipsis","outcome-completed","outcome-failed","outcome-cancelled","outcome-timed_out","outcome-lost"]) if(!css.includes(marker)) throw new Error(`LB-015 schema42 peer row/outcome styling missing: ${marker}`);
