...
const lifecycle = readFileSync("src-tauri/src/app/background.rs", "utf8");
...
const permissionBody = backend.slice(backend.indexOf("pub async fn set_permission_mode"), backend.indexOf("fn reconcile_explicit_permission"));
if (!permissionBody.includes("reconcile_explicit_permission(&lifecycle)?") || !lifecycle.includes("reconcile_permission_from_explicit_action") || !lifecycle.includes("PermissionReconcileAction::DisableBroker") || !lifecycle.includes("self.privilege.disable()")) throw new Error("LB-015 permission effects do not use the lifecycle-owned reconciliation entry");
